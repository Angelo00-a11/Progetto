...
public class GestoreOrdini {
    ...
    public GestoreOrdini() {
        ...
        // In questo modo la logica di creazione e collegamento delle Entity resta nel
        // package entity, mentre il codice tecnico di persistenza resta nel package
        // database.
    ...
    public Ordine registraOrdine(Cliente cliente, Indirizzo indirizzoConsegna, List<RigaCarrelloVirtuale> carrello,
            Long ristoranteId) {
        if (cliente == null || indirizzoConsegna == null || carrello == null
                || carrello.isEmpty() || ristoranteId == null) {
        ...
        o.setRistoranteOrdinato(ristoranteId);
        cliente.Ordina(o);
    ...
    /*
     * Questo metodo carica dal database un Ordine usando la sua Primary Key.
     * Attenzione: questo metodo recupera solo l'oggetto Ordine.
     * Se l'associazione con gli ordini è LAZY, la lista degli ordini potrebbe non
     * essere già caricata quando il metodo restituisce il risultato.
     */
    public Ordine cercaOrdinePerId(Long id) { // esiste anche la versione full (con tutte le associazioni)... ma non
                                              // credo mi serva
     ...
     * L'Ordine viene prima cercato tramite il suo id e, se esiste, vengono
     * modificati stato e indirizzo di consegna.
     */
    ...
    /*
     * Senza una configurazione specifica dell'associazione tra Ordine e Cliente, il
     * database/Hibernate non elimina automaticamente gli oggetti collegati.
     * Se nella classe Ordine fosse stata usata una configurazione corretta con
     * cascade, ad esempio:
     *
     ...
     *
     * allora, sarebbe bastato eliminare direttamente il Cliente perchè Hibernate
     * avrebbe eliminato automaticamente anche tutte gli ordini associati.
     */
    ...
    public void eliminaOrdiniDelCliente(Long idCliente) {
        ...
        // gp.elimina(Cliente.class, idCliente); Questo servirebbe eventualmente per
        // eliminare anche il cliente
    }
    ...
    public List<Ordine> cercaOrdiniDelCliente(Long idCliente) {
        ...
        return gp.cercaPerCampo(Ordine.class, "cliente.id", idCliente); // CERCA TUTTI GLI ORDINI ASSOCIATI AL CLIENTE
                                                                        // CON ID "IDCLIENTE" INTERROGANDO IL GP
    }
